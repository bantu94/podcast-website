			...
			foreach ($this->validationRules as $column => $rules) {
				...
				foreach ($rules as $rule) {
					...
					if(preg_match("/^same_as_/", $rule)) {
						// excute the same_as rule
						$col_name = str_replace("same_as_", "", $rule);

						if($data[$col_name] != $data[$column])
							$this->errors[$column] = ucfirst($column) . " does not match" . $col_name;
					}
				}
			}
		}
